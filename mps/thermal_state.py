...
class thermal_state(object):
    ...
    def network_from_cells(self, network_type, L, chi_MPO=None, params=None, bdry_vecs=None, method=None, T=None):      
        """
        ...
        if network_type == 'random_state' or network_type == 'circuit_MPS' or network_type == 'circuit_MPO':
            ...
            # if network_type is set to holoMPS:
                ...
                d = tensor[:,0,0].size # physical leg dimension (for random state)
                chi = tensor[0,:,0].size # bond leg dimension (for random state)
                tensor_list1 = [np.swapaxes(unitary[:,:,0,:],1,2) for unitary in unitary_list]  # bulk tensors of holoMPS structure
    ...
    def density_matrix(self, params, L, T, bdry_vecs=[None,None]):      
        ...
        state = thermal_state.network_from_cells(self,'circuit_MPO',L,None,params,bdry_vecs,None,T)
        probs_list = N * [thermal_state.prob_list(self,params,T)]
    ...
    def free_energy(self, params, state_type, L, Hamiltonian, T, chi_H=None, bdry_vecs1=None, bdry_vecs2=None, method=None, N_sample=None):
        """
        ...
        method: str 
           One of "thermal_state_class", "tenpy", or "MPO_tenpy" options. The option is 
           currently available for "random_state" ("MPO_tenpy" is set when MPO structure is 
           already constructed in TenPy).
       N_sample: int
        ...
        # for random-holoMPS-based structures:
            ...
            if method == 'thermal_state_class': 
                random_state = thermal_state.network_from_cells(self,'random_state',L,None,params,bdry_vecs1,'thermal_state_class',T) # random_state MPS
                Es = [thermal_state.expectation_value(random_state,'random_state',chi_H,MPO_Hamiltonian) for j in range(N_sample)] # sampling over energies
                E = np.mean(Es) # energy of system
                F = (E/N) - T*S # Helmholtz free energy   
            
            elif method == 'tenpy':
                MPO_Hamiltonian = thermal_state.network_from_cells(Hamiltonian,'MPO',N,chi_H,None,bdry_vecs2,'tenpy',T) # Hamiltonian MPO
                Es = [(MPO_Hamiltonian.expectation_value(random_state)).real for j in range(N_sample)] # sampling over energies
                E =  np.mean(Es) # energy of system
                F = (E/N) - T*S # Helmholtz free energy
            
            elif method == 'MPO_tenpy':
                random_state = thermal_state.network_from_cells(self,'random_state',L,None,params,bdry_vecs1,'tenpy',T) # random_state MPS
                Es = [(Hamiltonian.expectation_value(random_state)).real for j in range(N_sample)] # sampling over energies
                E =  np.mean(Es) # energy of system
                F = (E/N) - T*S # Helmholtz free energy
