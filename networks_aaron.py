...
import circuits.bond_circuits as bcircs
...
def basic_cost_function(circ_param_vals, qk_circuit, circ_params, target_matrix):
    param_dict = dict(zip(circ_params,circ_param_vals))
    # chi = len(target_matrix[0,:])
    unitary = bond_qk_unitary(qk_circuit, param_dict)
...
#        self.n_params = len(param_names)
        ...
        self.bond_prep_circ, self.bond_prep_param_names = bcircs.bond_star_circ(breg, label='bi')
        self.burnin_param_names = None
        self.burnin_param_vals = None
...
#        ulist = [self.sites[j][0].unitary(params) for j in range(self.l_uc)]
    ...
    def get_bond_steady_state(self, params, num_eigs = 0, debug = False):
        """
    
        Parameters
        ----------
        params : list or ndarray, optional
            DESCRIPTION: Parametes of the optimized holoMPS
        ...
        if debug:
            ...
            for dummy1,B_tensor in enumerate(tensors):
                print("tensor has shape {}".format(np.shape(B_tensor)))
                trans = np.tensordot(B_tensor,np.swapaxes(B_tensor.conj(),1,2),axes=([0],[0]))
                ### check that all tensors have same shape
                if dummy1 > 0:
                    assert np.shape(trans) == t_shape
                t_shape = np.shape(trans)
                dumb_thing = np.tensordot(B_tensor,B_tensor.conj(),axes=([0,2],[0,2])) 
                dumb_thing -= np.eye(len(dumb_thing[:,0])) 
                dumb_thing = np.abs(dumb_thing)
                print("things that should (probably) be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(dumb_thing.flatten().tolist())))
                print()
        ...
        for foo1,tensor in enumerate(tensors):
            t_mat = np.tensordot(tensor,np.swapaxes(tensor.conj(),1,2),axes=([0],[0]))
        ...
        ########################
        if debug:
            dumb_thing = np.abs(np.tensordot(transfer_matrix,np.eye(chi),axes=([2,3],[0,1])) - np.eye(chi))
            print("If right canonical, should be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(dumb_thing.flatten().tolist())))
            print()
        ...
        #### Sanity checks:
        ########################
        ### Confirmed: np.reshape(np.reshape(transfer2,(chi,chi,chi,chi)) == transfer_matrix,(chi**4))
        ...
        if num_eigs == 0:
            ...
            for foo2 in range(len(eigs)):
                lambdas.append(eigs[foo2])
                dm = np.reshape(vecs[foo2],(chi,chi))
                dm = dm / np.trace(dm)
            ...
            for foo3 in range(len(eigs)):
                lambdas.append(eigs[foo3])
                dm = np.reshape(vecs[foo3],(chi,chi))
                dm = dm / np.trace(dm)
                density_matrices.append(dm)
        ...
        if debug:
            ...
            for dumdum in range(len(lambdas)):
                ...
                dumb_thing = np.abs(dm - np.transpose(dm.conj()))
                print("If density matrices are Hermitean, should be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(np.abs(dumb_thing.flatten()).tolist())))
                dumb_thing = np.abs(dm - np.matmul(dm,dm))
                print("If density matrices are idempotent, should be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(np.abs(dumb_thing.flatten()).tolist())))
                tguy += lambdas[dumdum]*np.outer(dm,dm)
                idguy += np.outer(dm,dm)
                print()
            dumb_thing = np.abs(np.reshape(tguy - transfer2,(chi**4)))
            print("If eigenmodes reproduce transfer matrix, should be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(np.abs(dumb_thing.flatten()).tolist())))
            print()
            dumb_thing = np.abs(np.reshape(idguy - np.eye(chi**2),(chi**4)))
            print("If eigenmodes form complete set, should be zero: {}, {}, {}".format(np.average(dumb_thing.flatten()),np.std(dumb_thing.flatten()),max(np.abs(dumb_thing.flatten()).tolist())))
            print()
    ...
    def set_burn_in_params(self, steady_state, fun=basic_cost_function, args = None):
        ...
        fun : callable function
            A callable "cost function" fun( parameter_values, circuit, parameter_names, steady_state_density_matrix, optional arguments). 
            The first three arguments *MUST* be 1) circuit parameter values, 2) the circuit 3) parameter names / labels, 4) the target steady state DM for the bond register.
        ...
        if args is not None:
            fun = lambda  bond_param_vals, bond_circ, bond_param_names, ssdm, fun=fun: fun(bond_param_vals, bond_circ, bond_param_names, ssdm, *args)
        ### now fun is a three-argument function of some circuit (acting on the bond space) and some steady state density matrix.
        
        # breg = self.qregs[1]
        x0 = 0.03*np.random.randn(len(self.bond_prep_param_names))
        
        ### use some optimizer to minimize fun by varying parameters of circ
            
        ### OPTIMIZATION
        burn_result = minimize(fun, x0, args=(self.bond_prep_circ, self.bond_prep_param_names, steady_state), method='BFGS')
        burn_in_param_vals = burn_result.x
        burn_in_param_names = dict(zip(self.bond_prep_param_names , burn_in_param_vals))
        
        self.burnin_param_names = burn_in_param_vals
        self.burnin_param_vals = burn_in_param_names
